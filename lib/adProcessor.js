// Ad Processing module for Ad Vault (Railway deployment)
...
export async function processAd(url) {
  ...
  console.log(`[${tweetId}] Saving to storage...`);
  const videoKey = `videos/${tweetId}.mp4`;
  await saveFile(videoKey, videoData, 'video/mp4');
  ...
  // Step 6: Analyze with Gemini (if key available)
  let analysis = null;
  if (geminiKey) {
    console.log(`[${tweetId}] Analyzing with Gemini AI...`);
    analysis = await analyzeAdWithGemini(
      geminiKey,
